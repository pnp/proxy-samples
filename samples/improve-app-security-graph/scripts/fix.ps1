...
$envJs -replace 'const scopes = \[.*\];', 'const scopes = ["https://graph.microsoft.com/Tasks.Read"];' | Out-File .\src\env.js
...
Write-Output "Adding the Tasks.Read permission..."
npx -p @pnp/cli-microsoft365 -- "m365 entra app permission add --appId ${appId} --delegatedPermissions https://graph.microsoft.com/Tasks.Read --grantAdminConsent"

# remove the Group.ReadWrite.All permission
Write-Output "Removing the Group.ReadWrite.All permission..."
npx -p @pnp/cli-microsoft365 -- "m365 entra app permission remove --appId ${appId} --delegatedPermissions https://graph.microsoft.com/Group.ReadWrite.All --force"
